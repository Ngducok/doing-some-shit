...
function Starlight:CreateWindow(WindowSettings)
	...
	do
		...
		--[[ === MOBILE FIX + FLOATING TOGGLE (non-invasive) =====================
		     - Keep logic intact; only constrain drag bar hitbox and add a small
		       draggable square button to hide/show the UI.
		     ================================================================= ]]

		-- A) Constrain Drag bar so it doesn't swallow touches on the right-half
		do
			local dragBar = StarlightUI:FindFirstChild("Drag")
			if dragBar then
				local interact = dragBar:FindFirstChild("Interact")
				-- ensure the interact area doesn't capture the whole screen
				pcall(function()
					if interact and typeof(interact.Modal) == "boolean" then interact.Modal = false end
					if interact and typeof(interact.AutoButtonColor) == "boolean" then interact.AutoButtonColor = false end
				end)

				local function updateDragBounds()
					-- width equals current mainWindow width; sit 10px below it
					local cx = mainWindow.AbsolutePosition.X + mainWindow.AbsoluteSize.X/2
					local y  = mainWindow.AbsolutePosition.Y + mainWindow.AbsoluteSize.Y + 10
					dragBar.AnchorPoint = Vector2.new(0.5, 0)
					dragBar.Size        = UDim2.new(0, math.max(180, mainWindow.AbsoluteSize.X), 0, 14)
					dragBar.Position    = UDim2.fromOffset(cx, y)
					if interact then
				...
				updateDragBounds()
				mainWindow:GetPropertyChangedSignal("Size"):Connect(updateDragBounds)
				mainWindow:GetPropertyChangedSignal("Position"):Connect(updateDragBounds)
				game:GetService("RunService").RenderStepped:Connect(updateDragBounds)
			end
		end

		-- B) Add a draggable square toggle button (color matched to existing buttons)
		do
			local CoreGui = game:GetService("CoreGui")
			if not CoreGui:FindFirstChild("Starlight_FloatingToggle") then
				local gui = Instance.new("ScreenGui")
				gui.Name = "Starlight_FloatingToggle"
				gui.IgnoreGuiInset = true
				gui.ResetOnSpawn = false
				gui.Parent = CoreGui

				-- try to derive colors from Style2 button in resources; fallback if missing
				local btnColor = Color3.fromRGB(27, 29, 34)
				local strokeCol = Color3.fromRGB(65, 66, 77)
				pcall(function()
					local parts = StarlightUI.Resources.Elements["Tab_TEMPLATE"].ScrollingCollumnTemplate["Groupbox_Style1"]["Button_TEMPLATE_Style2"].PART_Backdrop
					btnColor  = parts.BackgroundColor3
					local st = parts:FindFirstChildOfClass("UIStroke")
					if st then strokeCol = st.Color end
				end)

				local toggle = Instance.new("TextButton")
				toggle.Name = "SquareToggle"
				toggle.Text = ""
				toggle.AutoButtonColor = false
				toggle.BackgroundColor3 = btnColor
				toggle.Size = UDim2.fromOffset(48, 48)
				toggle.Position = UDim2.new(1, -64, 0.5, -24) -- default right-middle
				toggle.ZIndex = 500
				toggle.Parent = gui
				...
				local function clampPos(px, py)
					local vp = viewport()
					local x = math.clamp(px, 6, vp.X - toggle.AbsoluteSize.X - 6)
					local y = math.clamp(py, 6, vp.Y - toggle.AbsoluteSize.Y - 6)
				...
				toggle.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
						...
						startPos = toggle.AbsolutePosition
						local c; c = input.Changed:Connect(function()
							if input.UserInputState == Enum.UserInputState.End then
								dragging = false
								c:Disconnect()
								local vp = viewport()
								local centerX = toggle.AbsolutePosition.X + toggle.AbsoluteSize.X/2
								local finalX = (centerX < vp.X/2) and 6 or (vp.X - toggle.AbsoluteSize.X - 6)
								toggle.Position = UDim2.fromOffset(finalX, toggle.AbsolutePosition.Y)
							end
						end)
					end
				end)
				UIS.InputChanged:Connect(function(input)
					if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
						...
						toggle.Position = clampPos(startPos.X + d.X, startPos.Y + d.Y)
					end
				end)
				RS.RenderStepped:Connect(function()
					-- keep within viewport when rotating screen
					local vp = viewport()
					local pos, size = toggle.AbsolutePosition, toggle.AbsoluteSize
					local x = math.clamp(pos.X, 6, vp.X - size.X - 6)
					local y = math.clamp(pos.Y, 6, vp.Y - size.Y - 6)
					toggle.Position = UDim2.fromOffset(x, y)
				end)

				-- click: hide/show window using existing Hide/Unhide (no logic change)
				toggle.Activated:Connect(function()
					if Starlight.Minimized then
						Unhide(mainWindow)
					else
						Hide(mainWindow, false, false, Starlight.WindowKeybind)
					end
				end)
			end
		end
		
		-- C) EXTRA GUARD: clamp any wide, thin overlay/hitbox under StarlightUI
		do
			local patterns = {
				["Drag"]=true, ["Interact"]=true, ["Overlay"]=true,
				["Blocker"]=true, ["Hitbox"]=true, ["ClickCatch"]=true,
				["TouchGrab"]=true, ["InputBlocker"]=true
			}
			local function isWideThin(obj)
				local sz = obj.AbsoluteSize
				return (sz.X > (mainWindow.AbsoluteSize.X + 50)) and (sz.Y <= 80)
			end
			local function clampCandidate(obj)
				local cx = mainWindow.AbsolutePosition.X + mainWindow.AbsoluteSize.X/2
				local y  = mainWindow.AbsolutePosition.Y + mainWindow.AbsoluteSize.Y + 10
				obj.AnchorPoint = Vector2.new(0.5, 0)
				obj.Size     = UDim2.new(0, math.max(180, mainWindow.AbsoluteSize.X), 0, math.clamp(obj.AbsoluteSize.Y, 10, 24))
				obj.Position = UDim2.fromOffset(cx, y)
				if obj:IsA("GuiButton") then
					pcall(function()
						if typeof(obj.Modal) == "boolean" then obj.Modal = false end
						if typeof(obj.AutoButtonColor) == "boolean" then obj.AutoButtonColor = false end
					end)
				end
			end
			local function sweep()
				for _, d in ipairs(StarlightUI:GetDescendants()) do
					if d:IsA("GuiBase2d") and d.Visible and patterns[d.Name] and isWideThin(d) then
						clampCandidate(d)
					end
				end
			end
			-- initial + reactive
			sweep()
			game:GetService("RunService").RenderStepped:Connect(sweep)
		end

		-- D) LOADING OVERLAY GUARD: if a full-screen loading/splash exists, push it behind the window
		do
			local RS = game:GetService("RunService")
			local function viewport()
				local cam = workspace.CurrentCamera
			...
			local loadNames = {
				["Loading"]=true, ["Loader"]=true, ["LoadingScreen"]=true, ["Splash"]=true,
				["Init"]=true, ["Boot"]=true, ["Fade"]=true, ["Cover"]=true, ["Backdrop"]=true
			}

			local function looksLikeFullscreen(g)
				local vp = viewport()
				local sz = g.AbsoluteSize
				return (sz.X >= vp.X * 0.6) and (sz.Y >= vp.Y * 0.5)
			end

			local function pushBehind(g)
				if typeof(g.ZIndex) == "number" then
					g.ZIndex = math.min(g.ZIndex, mainWindow.ZIndex - 1)
				end
				-- Prevent buttons from capturing focus/modal
				if g:IsA("GuiButton") then
					pcall(function()
						if typeof(g.Modal) == "boolean" then g.Modal = false end
						if typeof(g.AutoButtonColor) == "boolean" then g.AutoButtonColor = false end
					end)
				end
				-- Some Frames can still block input; lowering ZIndex keeps window on top
			end

			local function sweepLoading()
				for _, d in ipairs(StarlightUI:GetDescendants()) do
					if d:IsA("GuiObject") and d.Visible then
						if loadNames[d.Name] or looksLikeFullscreen(d) then
							pushBehind(d)
						end
					end
				end
			...
			sweepLoading()
			RS.RenderStepped:Connect(sweepLoading)
		end
--[[ === END PATCH ===================================================== ]]
